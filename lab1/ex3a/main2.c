...
void create_table(int size, FILE* file, void* handle);

void remove_table_block(int index, FILE* file, void* handle);
...
int main(int argc, char* argv[]){ // Testing programme.
    ...
    struct Array* (*create_array)(int size) = dlsym(handle, "create_array");
    void (*add_to_array)(struct Array* array, char* file_name) = dlsym(handle, "add_to_array");
    void (*count)(struct Array* array) = dlsym(handle, "count");
    int (*add_result)(struct Array* array) = dlsym(handle, "add_result");
    void (*print_array_rows)(struct Array* array) = dlsym(handle,"print_array_rows");
    ...
    create_table((int)size, file, handle);
    ...
    while(index < argc){
        ...
        if(i==1){
            ...
            if(check_is_number(argv[index])){
                ...
                create_table((int)table_size, file, handle);
            } else{
                print_integer_error_message();
                exit(1);
            }
            ...
            if(array == NULL){
                ...
                remove_table_block((int)table_index, file, handle);
            } else{
                perror("\n"
...
void remove_table_block(int index, FILE* file, void* handle){
    void (*remove_block)(struct Array* array, int index) = dlsym(handle, "remove_block");
    clockStart = times(&tmsStart);
